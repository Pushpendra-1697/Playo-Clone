...
function getRejectedUsersById(id, index) {
  return fetch(`${backend_url}/overview/rejected/${id}/${index}`).then((res) => res.json());
};
const RejectedUsers = () => {
  ...
  useEffect(() => {
    getRejectedUsersById(params.event_id, params.index).then((res) => setData(res)).catch((err) => console.log(err));
  }, []);
